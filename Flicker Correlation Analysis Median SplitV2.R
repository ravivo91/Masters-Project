...
# --- Directory Setup ---
# Define the main parent folder for all output using a full path
parentFolder <- "E:/iCloudDrive/Desktop/Rstudio/ERG_VEP/Flicker Median Split Analysis"
...
# --- NEW: Create a dedicated folder for distribution plots ---
...
# Read the Excel file
filePath <- "AllSubjectsResultsFlicker.xlsx" # or AllSubjectsResultsFlash.xlsx
...
# --- MODIFIED: Set up results storage with the new, detailed format ---
results <- data.frame(
...
# --- NEW: Generate and Save Distribution Plots (Q-Q Plots) ---
for (v in testVars) {
...
for (group_name in names(grouping_vars)) {
  ...
  for (v in testVars) {
    ...
    # Simple outlier removal (optional but good practice)
